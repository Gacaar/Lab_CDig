...
typedef so_observador_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * so_observador(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(so_observador_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://so_observador:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: so_observador -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (so_observador_DWork.EncoderInput0CartPositionEnco_o,
         so_observador_P.EncoderInput0CartPositionEncode, 100.0, -1)
        ;
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(so_observador_DWork.EncoderInput0CartPositionEnco_o)
          ;
