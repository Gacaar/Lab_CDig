...
typedef ControladorPI_LAB_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * ControladorPI_LAB(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ControladorPI_LAB_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ControladorPI_LAB -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (final_time >= 0.0 || final_time == RUN_FOREVER) {
    ...
    rtmSetTFinal(S, 4.9920000000000009E+000);
  }
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(ControladorPI_LAB_DWork.EncoderInput0CartPositionEnco_m)
          ;
