...
typedef ControlSemCanalslx_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * ControlSemCanalslx(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ControlSemCanalslx_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ControlSemCanalslx -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(ControlSemCanalslx_DWork.EncoderInput0CartPositionEnco_b)
          ;
